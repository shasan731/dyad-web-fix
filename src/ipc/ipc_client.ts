...
import { WebIpcRenderer } from "./web_ipc_renderer";
import type {
  ValidInvokeChannel,
  ValidReceiveChannel,
} from "./ipc_channels";
...
type IpcRendererLike = {
  invoke: (channel: ValidInvokeChannel, ...args: unknown[]) => Promise<any>;
  on: (
    channel: ValidReceiveChannel,
    listener: (...args: unknown[]) => void,
  ) => void;
  removeAllListeners: (channel: ValidReceiveChannel) => void;
  removeListener: (
    channel: ValidReceiveChannel,
    listener: (...args: unknown[]) => void,
  ) => void;
...
export class IpcClient {
  ...
  private ipcRenderer: IpcRendererLike;
  ...
  private constructor() {
    ...
    this.ipcRenderer =
      electronIpc ?? (new WebIpcRenderer() as unknown as IpcRendererLike);
  ...
  public static isWebRenderer(): boolean {
    return !IpcClient.isElectronRenderer();
  }
  ...
  public async takeScreenshot(): Promise<void> {
    if (!this.isElectron) {
      ...
      // @ts-ignore - ClipboardItem not in lib dom typings for some TS configs
      if (navigator.clipboard && window.ClipboardItem) {
        // @ts-ignore - ClipboardItem may be missing in older typings
      ...
      link.href = dataUrl;
      link.download = "dyad-screenshot.png";
      link.click();
      return;
    }

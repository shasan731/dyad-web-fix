...
interface DeviceFlowState {
  ...
  isPolling: boolean;
  window: InstanceType<typeof BrowserWindow> | null; // Reference to the window that initiated the flow
}
...
export function registerGithubHandlers() {
  ...
  ipcMain.handle(
    ...
    (event: IpcMainInvokeEvent, args: { owner: string; repo: string }) =>
  ...
  ipcMain.handle(
    "github:disconnect",
    (event: IpcMainInvokeEvent, args: { appId: number }) =>
      handleDisconnectGithubRepo(event, args),
  );
  ipcMain.handle(
    "github:clone-repo-from-url",
    async (event: IpcMainInvokeEvent, args: CloneRepoParams) => {

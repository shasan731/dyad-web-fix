...
export default function AppDetailsPage() {
  ...
  const [isChangeLocationDialogOpen, setIsChangeLocationDialogOpen] =
    useState(false);
  const [manualParentPath, setManualParentPath] = useState("");
  ...
  const handleChangeLocation = async () => {
    ...
    try {
      if (isWeb) {
        const trimmed = manualParentPath.trim();
        if (!trimmed) {
          showError("Please enter a parent directory path.");
          return;
        }
        await changeLocationMutation.mutateAsync({
          appId,
          parentDirectory: trimmed,
        });
  ...
  useEffect(() => {
    if (!isChangeLocationDialogOpen || !selectedApp) return;
    const currentPath = selectedApp.resolvedPath || selectedApp.path || "";
    const currentParentDir = currentPath
      ? currentPath.replace(/[/\\][^/\\]*$/, "")
      : "";
    setManualParentPath(currentParentDir);
  }, [isChangeLocationDialogOpen, selectedApp]);
          ...
          <div className="col-span-2">
            ...
            <div className="flex items-center gap-1">
              <Button
                ...
                title={isWeb ? "Copy path" : "Show in folder"}
              >
          ...
          <DialogContent className="max-w-sm p-4">
            ...
            {isWeb && (
              <div className="pt-2">
                ...
                <Input
                  value={manualParentPath}
                  onChange={(e) => setManualParentPath(e.target.value)}

...
export function NodePathSelector() {
  ...
  useEffect(() => {
    if (settings) {
      setManualNodePath(settings.customNodePath || "");
    }
  }, [settings?.customNodePath]);
  ...
  const handleApplyManualPath = async () => {
    const trimmed = manualNodePath.trim();
    try {
      await updateSettings({ customNodePath: trimmed || null });
      await IpcClient.getInstance().reloadEnvPath();

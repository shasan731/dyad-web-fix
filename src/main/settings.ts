...
export function decrypt(data: Secret): string {
  if (data.encryptionType === "electron-safe-storage") {
    if (!safeStorage.isEncryptionAvailable()) {
      logger.warn(
        "safeStorage not available; unable to decrypt stored secret. Returning empty string.",
      );
      return "";
    }
    try {
      ...
      logger.warn("Failed to decrypt stored secret. Returning empty string.", error);
      return "";
    }
  }
  return data.value;
